...
const getSubscribersList = async (req, res) => {
  try {
    ...
    const listPipeline = [
      ...
      // 4. Lookup AI Tests Generated by this student (isAiGenerated === true or title starts with AI Test)
      {
        $lookup: {
          from: "quizzes",
          let: { studentId: "$_id" },
                  ...
                  $and: [
                    { $eq: ["$createdBy", "$$studentId"] },
                    {
                      $or: [
                        ...
                        { $regexMatch: { input: "$title", regex: "^AI Test:", options: "i" } }
                      ]
                    }
                  ]
                }
